...
def detectFacesVideo(cap_source, wv, out_file_name, in_file_name, show, model_num, model_e):
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            emotion = EMOTIONS[model_num][pos_e]
            e_color = EMOTION_COLORS[model_num][pos_e]
    ...
    if not stats_emo == None and not stats_gen == None:
        if frames != 0:
            for emotion in stats_emo[model_num]:
                emotion[1] = emotion[1]/frames * 100
        stats_video = [stats_emo[model_num], stats_gen]
    else:
        stats_video = None
    
    return stats_video

def detectFacesImage(in_file_name, image, model_num, model_e):
    if not in_file_name:
        img = image
    else:
        try:
    ...
    for (x, y, w, h) in faces:
        ...
        stats_general_emotion[model_num][pos_e][1] += 1
    ...
    cv2.destroyAllWindows()
    stats_image = [stats_general_emotion[model_num], stats_general_gender, stats_specific_emotion, stats_specific_gender]
...
def takePhoto(model_num, model_e):
    try:
        cap = cv2.VideoCapture(1, cv2.CAP_DSHOW)
    except Exception as e:
        raise
        ...
        if k == ord('p'):
            while True:
                ...
                if k2 == ord('p'):
                    break
                if k2 == ord('s'):
                    stats_image = detectFacesImage(None, frame, model_num, model_e)
                    return stats_image
                if k2 == 27:
                    return None
...
def smartPhonePredict(model_num, model_e):
    url = 'http://10.26.197.64:8080/shot.jpg'
...
def main(debugging):
    ...
    while(True):
        ...
        if choice == '0' or choice == '1' or choice == '2' or choice == '3' or choice == '4' or choice == '5' or choice == '6':
            if choice == '0':
                print("Video input from Android device\n")
                try:
                    stats_video = smartPhonePredict(model, model_e)
                except Exception as e:
                    print("Could not read video from Android!")
                    continue
